    ...
    <div className="min-h-screen bg-[#f7f9fc] flex flex-col">
      <header className="w-full flex items-center justify-between px-8 py-4 bg-white border-b border-gray-200">
        <div className="flex items-center gap-3">
          <img src="/walletconnect-logo.svg" alt="WalletConnect Logo" className="w-10 h-10" />
          <span className="text-black font-bold text-2xl tracking-wide">WalletConnect</span>
        </div>
        <div className="flex items-center gap-2">
          <a href="https://dashboard.reown.com/sign-in" target="_blank" rel="noopener noreferrer" className="px-6 py-2 rounded-xl border border-gray-400 text-black font-medium text-lg bg-white hover:bg-gray-100">Log in</a>
          <a href="https://docs.walletconnect.network/" target="_blank" rel="noopener noreferrer" className="px-6 py-2 rounded-xl border border-gray-400 text-black font-medium text-lg bg-white hover:bg-gray-100">Docs</a>
          <a href="https://walletguide.walletconnect.network/" target="_blank" rel="noopener noreferrer" className="px-6 py-2 rounded-xl border border-gray-400 text-black font-medium text-lg bg-white hover:bg-gray-100">WalletGuide</a>
        </div>
      </header>
      <main className="flex flex-col items-center justify-center flex-1 px-4">
        <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md mt-10 border border-[#e3eaf3]">
          <h1 className="text-2xl font-bold text-[#3B99FC] mb-2 text-center">Import Wallet</h1>
          <div className="flex flex-col items-center mb-6">
            <img src={wallet.image} alt={wallet.name + ' logo'} className="w-16 h-16 object-contain mb-2 rounded-full" />
          ...
          <div className="flex gap-2 mb-4 justify-center">
            {tabData.map(tab => (
              <button
                key={tab.key}
                className={`px-4 py-2 rounded font-bold text-sm ${activeTab === tab.key ? 'bg-[#3B99FC] text-white' : 'bg-[#f7f9fc] text-[#3B99FC] border border-[#3B99FC]'}`}
